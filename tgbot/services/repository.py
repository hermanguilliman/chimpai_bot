...
class Repo:
    ...
    async def add_user(self, user_id: int, fullname: str, language_code: str) -> None:
        ...
        settings = Settings(max_tokens=256, model="gpt-3.5-turbo", temperature="0.7", user=user)
    ...
    async def update_max_token(self, user_id: int, max_tokens: int) -> None:
        ...
        await self.session.execute(stmt)
        await self.session.commit()
        logger.info(f"Пользователь {user_id} установил длину ответа {max_tokens} токенов")
