...
async def neural_handler(
    ...
    if settings is None:
        await message.answer('⚠️ <b>Настройки не найдены. Попробуйте выполнить /start</b>', parse_mode='HTML')
        return
    if not settings.api_key:
        await message.answer("⚠️ <b>Сначала нужно установить api ключ!</b>", parse_mode='HTML')
        return
    if personality:
